...
gnome: prepare
	./interpolate-preseed.sh -r -u $$(cat default-user) -p essential-cli-tools,cli-tools,desktop,desktop-extra,dutch-desktop,docker -o build/gnome.cfg -t gnome -c sudo-nopasswd,prepare-education-box,docker,google-chrome,tmux-conf,no-gnome-initial
...
complete: prepare
	./interpolate-preseed.sh -r -u $$(cat default-user) -p $(ALL_PACKAGES) -o build/complete.cfg -t gnome -c sudo-nopasswd,prepare-education-box,docker,google-chrome,uu-add-origins,uu-activate,tmux-conf,no-gnome-initial,vscode

personal: prepare
	./interpolate-preseed.sh -r -a -p $(ALL_PACKAGES) -o build/personal.cfg -t gnome -c sudo-nopasswd,prepare-education-box,docker,google-chrome,uu-add-origins,uu-activate,tmux-conf,no-gnome-initial,vscode

steven: prepare
	./interpolate-preseed.sh -r -u steven -p $(ALL_PACKAGES) -o build/steven.cfg -t gnome -c sudo-nopasswd,prepare-education-box,docker,google-chrome,uu-add-origins,uu-activate,tmux-conf,no-gnome-initial,vscode
